...
def load_results(folder: str) -> dict:
    ...
    When tasks are run one at a time, each produces its own results JSON
    in a separate subfolder. We merge all of them into a single dict.
    """
    folder = Path(folder)
    ...
    # Merge results from all JSON files
    merged = {"results": {}}
